...
export default function Doctor() {
  ...
  useEffect(() => {
    ...
    const handler = (p: { doctorId: string }) => {
      if (p?.doctorId === doctorId) {
        qc.invalidateQueries({ queryKey: ['queue', 'doctor', doctorId] })
  ...
  const start = useMutation({
    mutationFn: (id: string) => api.doctorStart(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['queue', 'doctor', doctorId] }),
  })
  const complete = useMutation({
    mutationFn: (id: string) => api.doctorComplete(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['queue', 'doctor', doctorId] }),
  })
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-white dark:from-slate-900 dark:to-slate-950 text-slate-900 dark:text-slate-100 p-6 animate-fadeIn">
      <div className="max-w-6xl mx-auto grid gap-6">
        ...
        <div className="flex flex-wrap items-center gap-4 animate-slideIn">
          <h2 className="text-3xl font-bold bg-gradient-to-r from-sky-600 to-blue-600 bg-clip-text text-transparent">Doctor Dashboard</h2>
          <select
          ...
          <button onClick={() => refetch()} className="ml-auto px-4 py-2 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 text-white font-medium hover:from-sky-600 hover:to-blue-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl">
            Refresh
          </button>
        </div>
        ...
        <div className={`p-8 rounded-3xl bg-white/90 dark:bg-slate-800/90 shadow-2xl border border-slate-200 dark:border-slate-700 backdrop-blur animate-scaleIn ${flash ? 'flash-once' : ''}`}>
          ...
          {isLoading && (
            <div className="flex items-center justify-center gap-3 py-12">
              ...
              <span className="text-lg text-slate-600 dark:text-slate-400">Loading patients...</span>
            </div>
          )}
          {!isLoading && queue && queue.length === 0 && (
            <div className="text-center py-12">
              <div className="text-xl font-semibold text-slate-700 dark:text-slate-300 mb-2">No Patients in Queue</div>
          ...
          <div className="grid gap-4">
            {queue?.map((q, idx) => {
              ...
              const canStart = state === 'ARRIVED'
              const canComplete = state === 'BEING_TREATED'

              return (
                <div key={q.appointmentId} className={`flex items-center justify-between p-5 rounded-2xl border-2 ${bgColor} shadow-lg hover:shadow-xl transform hover:scale-[1.01] transition-all duration-300 animate-slideIn`} style={{animationDelay: `${idx * 0.05}s`}}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => start.mutate(q.appointmentId)}
                      disabled={!canStart || start.isPending}
                      className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transform hover:scale-105 active:scale-95 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Start Treatment
                    </button>
                    <button
                      onClick={() => complete.mutate(q.appointmentId)}
                      disabled={!canComplete || complete.isPending}
                      className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 text-white font-medium hover:from-emerald-600 hover:to-green-700 transform hover:scale-105 active:scale-95 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Complete
                    </button>
                  </div>
